...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: report, error } = await supabase.from("reports").select("id, data").eq("id", id).maybeSingle();

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    if (!report) return NextResponse.json({ error: "Report nicht gefunden." }, { status: 404 });

    const origin = new URL(req.url).origin;
    const previewUrl = new URL("/api/pdf/tagesbericht-rhein-main-link", origin);
      ...
      return NextResponse.json(
        {
          ...
          detail: detail || `POST /api/pdf/tagesbericht-rhein-main-link -> ${res.status}`,
        },
        { status: 500 }
      );
    }
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="tagesbericht-rhein-main-link-${id}.pdf"`,
