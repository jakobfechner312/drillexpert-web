...
export async function GET() {
  ...
  return NextResponse.json({
    ok: true,
    route: "/api/pdf/excel/pumpversuch",
    method: "POST",
    note: "Exportiert Pumpversuch als PDF.",
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    const pdfBytes = await generateExcelProtocolPdf("Pumpversuch", payload ?? {});
    const stamp = new Date().toISOString().slice(0, 10);
    const fileName = `Pumpversuch-${stamp}.pdf`;
