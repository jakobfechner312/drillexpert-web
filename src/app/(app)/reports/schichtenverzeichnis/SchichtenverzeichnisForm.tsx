...
export default function SchichtenverzeichnisForm({
  ...
  const setFilterPairEntries = useCallback(
    (
      ...
      const cappedEntries = entries.slice(0, 2);
      const nextVisibleRows = Math.max(1, Math.min(2, cappedEntries.length));
      setFilterPairRowCounts((prev) => ({ ...prev, [pairId]: nextVisibleRows }));
      setFilterRows((prev) => {
        const nextLength = Math.max(prev.length, cappedEntries.length, 1);
        const next = Array.from({ length: nextLength }, (_, idx) => ({
          ...emptyFilterRow(),
          ...(prev[idx] ?? {}),
        }));
        for (let i = 0; i < nextLength; i += 1) {
          next[i][vonKey] = cappedEntries[i]?.von ?? "";
          next[i][bisKey] = cappedEntries[i]?.bis ?? "";
        }
        return next;
      });
    },
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const fullName = String(
        ((userRes?.user?.user_metadata as { full_name?: unknown } | undefined)?.full_name ?? "")
      ).trim();
      if (!fullName) return;
      setData((prev) => {
        if (String(prev.bohrmeister ?? "").trim()) return prev;
        return { ...prev, bohrmeister: fullName };
      });
    })();
    return () => {
      mounted = false;
